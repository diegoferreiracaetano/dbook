...
// Shared ("singleton") Testcontainers Postgres + Redis: started once per test JVM, reused
// by every test class that extends this one, instead of one container per class (slow) or
// requiring a manually-run `docker compose up -d` (what M1-M3 did — fine for local dev,
// but CI has no such container running). Testcontainers' own reaper (Ryuk) stops them
